...
class Log
{
    ...
    public function writeDataBase($event)
    {
        //建立yyc3資料庫連線
        ...
        $event = iconv("UTF-8", "big5", $event);

        $fields = "EVENT, KEY_OP, KEY_DT";

        $values = "'$event', '$userid', sysdate";
        
        $db->insStmt("GreetingCardLog", $fields, $values);
    }

    public function writeCardDataBase($event, $Receive_Name, $contentTextarea, $Send_Name)
    {
        //建立yyc3資料庫連線
        $db = new Database('oracle', DB_HT_3, '1521',DB_SD_3);
        $db->initDB(DB_UR_3, DB_PD_3);
        ...
        $Receive_Name = iconv("UTF-8", "big5", $Receive_Name);

        $contentTextarea = iconv("UTF-8", "big5", $contentTextarea);

        $Send_Name = iconv("UTF-8", "big5", $Send_Name);

        $fields = "EVENT, KEY_OP, KEY_DT, Receive_Name, contentTextarea, Send_Name";

        $values = "'$event', '$userid', sysdate, '$Receive_Name', '$contentTextarea', '$Send_Name'";
